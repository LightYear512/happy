...
/**
 * Poll `predicate` until it returns truthy or `timeoutMs` elapses. Replaces the
 * fragile `await sleep(N); expect(...)` pattern: on a fast machine the assertion
 * fires as soon as the watcher emits, on a loaded machine we wait up to the
 * timeout instead of failing the whole suite when N happened to be too short.
 *
 * Failure messages include the predicate label so we can locate which step in a
 * long sequence ran out of time.
 */
async function waitFor(
  ...
  // One last try so the error reflects the latest state, not a stale read.
  if (predicate()) return
...
describe('sessionScanner', () => {
  ...
  beforeEach(async () => {
    // mkdtemp guarantees a unique suffix even when two test files start in the
    // same millisecond (the previous `Date.now()` template could collide under
  ...
  it('should process initial session and resumed session correctly', async () => {
    ...
    await waitFor(() => collectedMessages.length >= 1, 'phase1 first user message')
    ...
    await waitFor(() => collectedMessages.length >= 2, 'phase1 assistant response')
    ...
    // The historical user + assistant messages (lines 1-2) are deduplicated
    // because they have same UUIDs as Phase 1 emissions.
    await waitFor(() => collectedMessages.length >= phase1Count + 1, 'phase2 summary emitted')
    ...
    const beforeUserAsk = collectedMessages.length
    await appendFile(sessionFile2, lines2[3] + '\n')
    await waitFor(() => collectedMessages.length > beforeUserAsk, 'phase2 user ask emitted')
    ...
    const beforeTail = collectedMessages.length
    const tailWriteCount = lines2.length - 4
    for (let i = 4; i < lines2.length; i++) {
      await appendFile(sessionFile2, lines2[i] + '\n')
    }
    // Wait for at least `tailWriteCount` more emissions (some lines may be filtered/dedupped
    // but tool_use + tool_result + final assistant msg should each emit at minimum).
    await waitFor(
      () => collectedMessages.length >= beforeTail + Math.min(3, tailWriteCount),
      'phase2 tool sequence emitted',
    )
