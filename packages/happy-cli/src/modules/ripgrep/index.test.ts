...
// Detect whether a real ripgrep binary is available (not the mock fallback).
const versionResult = await run(['--version']);
const realRipgrepAvailable = versionResult.exitCode === 0 && !versionResult.stdout.includes('(mock)');
...
describe('ripgrep low-level wrapper', () => {
    ...
    describe.skipIf(!realRipgrepAvailable)('search operations (requires real ripgrep binary)', () => {
